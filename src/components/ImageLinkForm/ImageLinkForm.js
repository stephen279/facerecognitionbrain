	...
	<p className = 'f3'>
			...
			<div className='center'>
               ...
               {/*<input className='f4 pa2 w-390 center' type = 'file' />*/}
				<p>
		<select id = "symptoms" class="w-80 db h2 f4 bg-near-white ba b--sliver gray" onChange={handleSymptomsChange} name="" onChange={onInputChange} > 
      <option value="⬇️ ⬇️">  Symptoms one </option>
      ...
      {symptoms.map((symptom) => <option value={symptom.ID}>{symptom.Name}</option>)}
						...
						<select id = "symptoms1" class="w-80 db h2 f4 bg-near-white ba b--sliver gray" onChange={handleSymptomsChange}  name="" onChange={onInputChange}  > 
      <option value="">  Symptoms two</option>
            {/* Mapping through each fruit object in our fruits array
          and returning an option element with the appropriate attributes / values.
         */}
      {symptoms.map((symptom) => <option value={symptom.ID}>{symptom.Name}</option>)}
...
<button className='f7 link dim ph3 pv1 mb1 dib white bg-light-purple' id="submit" onClick={onButtonSubmit}>Diagnose</button>
